...
outcome = pd.DataFrame(np.zeros((len(df),6)))
outcome.columns = ["year", "surface", "2ndRnWon%", "svWon%", "1stRnWon%", "outcome"]

#randomly rearrange data frame
for i in range(len(df)):
  p = np.random.random()
  if p < .5:
    temp = df["winner_name"].iloc[i]
    df.loc[i, "winner_name"] = df["loser_name"].iloc[i]
    df.loc[i, "loser_name"] = temp
    outcome.loc[i, "outcome"] = 0

#statistic to append--randomly arranges winners and losers
for i in range(len(df)):
  outcome.loc[i, "year"] = df["year"].iloc[i]
  outcome.loc[i, "surface"] = df["surface"].iloc[i]
  if outcome["outcome"].iloc[i] == 1:
...
stat1 = generateStats(df["winner_name"].iloc[0], df["loser_name"].iloc[0], df["year"].iloc[0], "2ndRnWon%")
stat2 = generateStats(df["winner_name"].iloc[0], df["loser_name"].iloc[0], df["year"].iloc[0], "1stRnWon%")
stat3 = generateStats(df["winner_name"].iloc[0], df["loser_name"].iloc[0], df["year"].iloc[0], "svWon%")

for i in range(1,len(df)):
  stat1 = stat1.append(generateStats(df["winner_name"].iloc[i], df["loser_name"].iloc[i], 
                                    df["year"].iloc[i], "2ndRnWon%"), ignore_index=True) 
for i in range(1,len(df)):
  stat2 = stat2.append(generateStats(df["winner_name"].iloc[i], df["loser_name"].iloc[i], 
                                    df["year"].iloc[i], "1stRnWon%"), ignore_index=True)
for i in range(1,len(df)):
  stat3 = stat3.append(generateStats(df["winner_name"].iloc[i], df["loser_name"].iloc[i], 
                                    df["year"].iloc[i], "svWon%"), ignore_index=True)
